...
async function handleRequest(req: NextRequest, params: { path: string[] }) {
  ...
  try {
    ...
    if (subPath === 'admin/metrics' && method === 'GET') {
      ...
      if (supabase) {
        try {
          ...
          if (dbU && dbU.length > 0) {
            totalU = dbU.length;
            pendingU = dbU.filter((u: any) => u.status === 'Pending').length;
            activeU = dbU.filter((u: any) => u.status === 'Active').length;
          }
        } catch (e) {}
      }
    ...
    // 12. DATA: TENDER ANALYZE (GENUINE AI PDF EXTRACTION & NON-TENDER REJECTION ENGINE)
    if (subPath === 'tender/analyze' && method === 'POST') {
      ...
      // 1. Extract raw text from uploaded PDF buffer if available
      ...
      const textCombined = `${extractedPdfText} ${titleInput} ${filename}`.toLowerCase();

      // 2. DETECT NON-TENDER DOCUMENTS (Invoices, Receipts, Utility Bills, Personal Letters)
      const isInvoiceOrReceipt = 
        (textCombined.includes('tax invoice') || textCombined.includes('invoice number') || textCombined.includes('billing id') || textCombined.includes('subtotal in inr') || textCombined.includes('google ireland') || textCombined.includes('google one') || textCombined.includes('payment receipt') || textCombined.includes('statement of account')) &&
        (!textCombined.includes('tender') && !textCombined.includes('notice inviting') && !textCombined.includes('eligibility criteria') && !textCombined.includes('bid document') && !textCombined.includes('turnover requirement'));

      if (isInvoiceOrReceipt) {
        const rejectionReport = {
          tender_id: `inv-rejected-${Date.now()}`,
          tender_title: `INVALID DOCUMENT: ${filename}`,
          ...
          executive_summary: `Document Verification Failed: The uploaded document '${filename}' is identified as a Commercial Tax Invoice / Billing Receipt (Google Ireland / Service Invoice). It does NOT contain tender bidding specifications, Notice Inviting Bids (NIB), or eligibility qualification clauses. Please upload an official Government or Corporate Tender / RFP PDF.`,
          desire_alone: {
            score: 0,
            status: 'Ineligible (Non-Tender File)',
            fulfilled_pct: '0.0%'
          },
          jv_alone: {
            score: 0,
          ...
          clauses_breakdown: [
            {
              clause_no: 'N/A',
              clause_title: 'Document Type Validation Error',
              requirement_type: 'Compliance',
              tender_requirement: 'Official Tender / RFP Document with Technical & Financial Qualification Clauses',
              required_value: 'Official Tender / NIB Document',
              desire_value: 'Uploaded file is a Commercial Tax Invoice / Bill (₹489.00)',
              jv_value: 'N/A',
              combined_value: 'N/A',
              applicable_jv_rule: 'Non-Tender Documents cannot be evaluated for bidding',
              status: 'NOT MATCHING',
              fulfilled_pct: '0.0%',
              gap_notes: 'System detected Google Ireland Tax Invoice instead of a Government/Municipal Tender Document. No tender evaluation can be performed on an invoice.',
              required_doc: 'Valid Tender Document (NIB / RFP / Volume 1)',
              page_ref: 'Page 1'
            }
          ],
          parameter_matrix: [
            {
              ...
              company_capability: 'Tax Invoice Uploaded',
              status: 'Not Met',
              gap_notes: 'Uploaded file is a billing invoice, not a tender.'
            }
          ],
          jv_rules_audit: [],
          summary_counts: {
      ...
      // 3. Fetch Permanent Company Master Data from Database
      ...
      // 4. Identify State & Tender Category from extracted text
      const isJunagadhTender = textCombined.includes('junagadh') || textCombined.includes('gwssb') || textCombined.includes('8.34');
      const isAlwarTender = textCombined.includes('alwar') || textCombined.includes('rudsico') || textCombined.includes('pkg 44') || textCombined.includes('36.53');
      const isSolarTender = textCombined.includes('solar') || textCombined.includes('kusum') || textCombined.includes('pv');
      ...
      if (isJunagadhTender) {
        category = 'ESCO';
        tenderTitle = 'Gujarat GWSSB Junagadh O&M Water Supply Package (Cost: ₹8.34 Cr)';
        clausesBreakdown = [
        ...
        category = 'STP';
        tenderTitle = 'RUDSICO Alwar Town Sewerage Package 44 (Cost: ₹36.53 Cr)';
        clausesBreakdown = [
        ...
        // GENERAL DYNAMIC TENDER PARSER
        category = isSolarTender ? 'KUSUM' : category;
        tenderTitle = titleInput || `Tender Document - ${category}`;
        clausesBreakdown = [
