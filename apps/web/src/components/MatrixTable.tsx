...
export const MatrixTable: React.FC<MatrixTableProps> = ({ parameters }) => {
  const getStatusBadge = (status: string) => {
    ...
    if (s.includes('partially')) {
      return (
        <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1 rounded-full font-bold bg-amber-50 text-amber-900 font-bold border border-amber-200">
    ...
    <div className="glass-card rounded-2xl p-6 border border-slate-200">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-slate-200 pb-4">
        ...
        <div className="flex items-center space-x-2 text-xs font-mono px-4 py-2 rounded-xl bg-slate-50/80 border border-slate-200">
          <span className="text-emerald-800 font-bold font-bold">{metCount} Met</span>
          <span className="text-slate-700 font-medium">/</span>
          <span className="text-slate-900">{totalCount} Total</span>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-white/5">
            {parameters.map((item, idx) => (
              <tr key={idx} className="hover:bg-white/5 transition-colors group">
                <td className="py-4 px-4 font-semibold text-slate-900 group-hover:text-teal-800 transition-colors max-w-[200px]">
                  {item.parameter}
                </td>
                ...
                <td className="py-4 px-4 text-xs text-slate-700 font-medium italic">
                  {item.gap_notes || '—'}
                </td>
              </tr>
            ))}
